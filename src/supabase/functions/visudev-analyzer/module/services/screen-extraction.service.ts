...
/**
 * Orchestrates screen extraction: framework-specific routing extraction (Next/React Router/Nuxt/etc.)
 * delegates navigation-link extraction and modal/tab/dropdown detection to dedicated services (SRP).
 * Dependency Inversion: navExtractor and stateTargetExtractor can be injected for tests.
 */
export class ScreenExtractionService {
    ...
    files.forEach((file) => {
      ...
      if (match) {
        ...
        screens.push({
          ...
          type: "page",
          flows: [],
          navigatesTo: this.navExtractor.extractNavigationLinks(file.content),
  ...
  public extractReactRouterScreens(files: FileContent[]): Screen[] {
    ...
    for (const file of files) {
      ...
      const navigatesTo = this.navExtractor.extractNavigationLinks(
        file.content,
      );
  ...
  public extractStateBasedScreens(files: FileContent[]): Screen[] {
    ...
    while ((m = caseRegex.exec(appFile.content)) !== null) {
      ...
      screens.push({
        ...
        type: "view",
        flows: [],
        navigatesTo: this.navExtractor.extractNavigationLinks(appFile.content),
    ...
    if (validPagesMatch) {
      ...
      for (const page of pageList) {
        ...
        screens.push({
          ...
          type: "view",
          flows: [],
          navigatesTo: this.navExtractor.extractNavigationLinks(
            appFile.content,
          ),
  ...
  public extractSinglePageFallback(
    ...
    return [{
      ...
      flows: [],
      navigatesTo: appFile
        ? this.navExtractor.extractNavigationLinks(appFile.content)
        : [],
  ...
  public extractScreensHeuristic(files: FileContent[]): Screen[] {
    ...
    files.forEach((file) => {
      ...
      const modulesMatch = file.path.match(
        /^(?:src\/)?modules\/([^/]+)\/pages\/([^/]+)\.(tsx?|jsx?)$/i,
      );
      if (modulesMatch) {
        const moduleName = modulesMatch[1].toLowerCase();
        const fileName = modulesMatch[2];
        const path = moduleName === "shell" ? "/" : `/${moduleName}`;
  ...
  public extractPageLikeScreens(
    ...
    this.pageLikeExtractor.extractPageLikeScreens(screens, files);
  }
